...
import { getToken } from "@/utils/auth";
  ...
  methods: {
    handleUpload(raw){
      ...
      let flag = false
      // if(raw.uid)
      let index = raw.file.uid
      this.fileList.forEach( ele => {
        if(ele.uid === index){
          // this.fileList.splice(this.fileList.indexOf(ele),1)
          console.log("该图片已存在");
          flag = true
    ...
    async subfileForm(){
      ...
      this.fileList.forEach(file => {
        ...
        if(res.data.code===200){
          console.log('yes')
          this.resList.push({ name: res.data.fileName, url: res.data.url });
      ...
      // console.log(formData.get('file'))       
      // console.log(config);
      // axios.post( process.env.VUE_APP_BASE_API + "/common/upload", this.fileList, config).then(res=>{
      //   console.log(res);
  ...
  // 上传失败
    ...
    handleRemove(file) {
      ...
      this.resList.forEach( ele => {
        if(ele.uid === index){
          this.resList.splice(this.resList.indexOf(ele),1)
        }
      })
      this.fileList.forEach( ele => {
        if(ele.uid === index){
